...
def validate(fase: str):
    ...
    if not products:
        print(f"Geen producten gevonden voor fase {fase}.")
        return

    print(f"Valideren: {len(products)} producten (fase {fase})\n")
    ...
    for product in products:
        ...
        for field in REQUIRED_FIELDS:
            ...
            if val is None or str(val).strip() in ("", "None", "nan"):
                issues.append(f"leeg verplicht veld: {field}")
                set_review = True
    ...
    print(f"  + {counts.get('ready', 0)} producten klaar voor export")
    print(f"  ~ {counts.get('review', 0)} producten voor handmatige controle -> {review_path}")
    if auto_fixed:
        print(f"  * {auto_fixed} velden automatisch gecorrigeerd")
    if warnings:
        print(f"\n  Waarschuwingen ({len(warnings)}):")
        for w in warnings[:20]:
            print(f"    - {w}")
        if len(warnings) > 20:
            print(f"    ... en {len(warnings) - 20} meer (zie review CSV)")
