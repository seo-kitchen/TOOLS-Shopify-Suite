...
def load_foto_export(path: str) -> dict:
    """
    ...
    if not col_sku:
        print(f"  WAARSCHUWING: geen SKU-kolom gevonden in foto-export, foto's overgeslagen.", file=sys.stderr)
        return {}
...
def ingest(file_path: str, fase: str, fotos_path: str | None):
    ...
    print(f"Inladen: {path.name} (fase {fase})")
    df = pd.read_excel(path, dtype=str)
    df = resolve_columns(df)

    missing = validate_columns(df)
    if missing:
        print(f"FOUT: Verplichte kolommen ontbreken: {missing}", file=sys.stderr)
        sys.exit(1)

    print(f"  {len(df)} rijen gevonden in Excel")

    foto_map = {}
    if fotos_path:
        foto_map = load_foto_export(fotos_path)
        print(f"  Foto-export: {len(foto_map)} SKUs met foto's geladen")
    ...
    if not rows:
        print("FOUT: Geen geldige producten om te laden.")
        sys.exit(1)
    ...
    for i in range(0, len(rows), 100):
        ...
        print(f"  Batch {i // 100 + 1}: {len(batch)} producten geladen")
    ...
    if all_warnings:
        print("Controleer de waarschuwingen hierboven voor je doorgaat.")
