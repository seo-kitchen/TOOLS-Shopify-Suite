...
# Forceer UTF-8 op Windows stdout (producttitels kunnen unicode zoals zero-width spaces bevatten)
try:
...
# Resize target: max 1200 breed x 1601 hoog (matcht Hextom Image Bulk Edit default)
MAX_WIDTH = 1200
MAX_HEIGHT = 1601
JPEG_QUALITY = 85
MAX_PACKSHOTS = 5
MAX_LIFESTYLES = 2
PHASH_AUTO_MATCH = 3   # diff <= 3 = zeker dezelfde foto (auto-reuse)
PHASH_BORDERLINE = 10  # diff 4-10 = mogelijk dezelfde, wordt gelogd als WAARSCHUWING maar toch geupload
SHOP_DOMAIN = "interieur-shop.nl"  # voor live-image fetch via storefront /products/{handle}.json
...
SKU_RE = re.compile(r"[BV]\d{7}(?:-\d{3})?[A-Z]?")
...
def classify(filename: str) -> tuple[str, int, str]:
    ...
    if has_ecom:
        ...
        priority = 6
    elif re.fullmatch(r"[BV]\d{7}(?:-\d{3})?[A-Z]?", no_pref):
        priority = 2
...
def resize_to_bytes(path: Path) -> tuple[bytes, int, int, int, int]:
    """Return (jpeg_bytes, orig_w, orig_h, new_w, new_h). Target: max 1200x1601, JPEG q85 progressive."""
...
def phash_local(path: Path) -> imagehash.ImageHash | None:
    try:
        with Image.open(path) as img:
            return imagehash.phash(img)
    except Exception as e:
        print(f"    phash fout voor {path.name}: {e}")
        return None


def phash_from_url(url: str, cache: dict) -> imagehash.ImageHash | None:
    """Download + pHash. Cached per URL."""
    if url in cache:
        return cache[url]
    try:
        ...
        print(f"    phash URL fout ({url[:60]}): {e}")
        cache[url] = None
        return None


def fetch_live_shopify_images(handle: str) -> list[str]:
    """Haal alle image URLs op voor een product via storefront /products/{handle}.json.
    Return [] bij 404 (product niet live) of andere fout."""
    if not handle:
        return []
    try:
        url = f"https://{SHOP_DOMAIN}/products/{handle}.json"
        r = requests.get(url, timeout=15)
        if r.status_code != 200:
            return []
        images = r.json().get("product", {}).get("images", []) or []
        return [im.get("src") for im in images if im.get("src")]
    except Exception as e:
        print(f"    storefront fetch fout voor {handle}: {e}")
        return []
...
def normalize_photo_key(name_or_url: str) -> str:
    ...
    s = re.sub(r"\.(jpg|jpeg|webp|gif|png)$", "", s)
    s = re.sub(r"-jpg$", "", s)                    # Shopify's redundant -jpg suffix
    s = re.sub(r"^(ecom_ ?|b2b_ ?)", "", s)        # strip prefix
    s = re.sub(r"_\d{6,8}_", "_", s)               # datum-token zoals _2502026_ tussen SKU en positie
...
def execute_plan(sb, plan: dict, db_products: dict, filter_skus: list[str] | None, dry: bool):
    """Override-all met 2-staps same-photo detectie.
      Stap 1 (snel): filename-normalisatie match
      Stap 2 (zeker): perceptual hash (download Shopify/Supabase URL, vergelijk pHash)
    ...
    for idx, sku in enumerate(skus, 1):
        ...
        # Bestaande URLs: LIVE Shopify images (via storefront JSON) + Shopify CDN URLs uit DB.
        # Supabase URLs worden genegeerd (die zijn van ons, overschrijven met kleine versies).
        live_images = fetch_live_shopify_images(prod.get("handle"))
        db_shopify = [
            prod.get(s) for s in PHOTO_SLOTS
            if prod.get(s) and "supabase.co/storage" not in (prod.get(s) or "")
        ]
        existing_urls: list[str] = list({u for u in (live_images + db_shopify) if u})
        existing_by_key: dict[str, str] = {normalize_photo_key(u): u for u in existing_urls}
        print(f"[{idx}/{len(skus)}] {sku}  live Shopify images: {len(live_images)}  | DB Shopify URLs: {len(db_shopify)}  | dedup-pool: {len(existing_urls)}")

        # pHash cache voor bestaande URLs (lazy - alleen als fallback nodig)
        url_phash_cache: dict[str, imagehash.ImageHash | None] = {}

        final_slots: dict[str, str | None] = {k: None for k in PHOTO_SLOTS}

        def find_same_existing(src_path: Path, src_name: str) -> tuple[str, str] | None:
            """Zoekt match in existing_urls. Return (url, method) of None.
            - filename-match: altijd reuse (exact same file)
            - pHash diff <= 3: auto-reuse (near-identical pixels)
            - pHash diff 4-10: logt WARN maar returned None (upload toch, user kan checken)
            """
            norm = normalize_photo_key(src_name)
            if norm in existing_by_key:
                return existing_by_key[norm], "filename"
            local_hash = phash_local(src_path)
            if local_hash is None:
                return None
            best_diff = 64
            best_url = None
            for url in existing_urls:
                if url in url_phash_cache and url_phash_cache[url] is None:
                    continue
                remote_hash = phash_from_url(url, url_phash_cache)
                if remote_hash is None:
                    continue
                diff = local_hash - remote_hash
                if diff < best_diff:
                    best_diff = diff
                    best_url = url
            if best_url is None:
                return None
            if best_diff <= PHASH_AUTO_MATCH:
                return best_url, f"phash(diff={best_diff})"
            if best_diff <= PHASH_BORDERLINE:
                print(f"    ?? WAARSCHUWING: {src_name} lijkt op bestaande (diff={best_diff}): {best_url.split('/')[-1][:50]} -> toch uploaden")
            return None

        def process(ph_info: dict, slot_type: str, slot_idx: int) -> None:
            slot_key = f"photo_{slot_type}_{slot_idx}"
            src = Path(ph_info["path"])
            match = find_same_existing(src, src.name)
            if match is not None:
                url, method = match
                host = "shopify" if "shopify" in url else ("supabase" if "supabase" in url else "other")
                print(f"[{idx}/{len(skus)}] {sku} {slot_type}_{slot_idx}  REUSE ({host}, {method}): {src.name}")
