...
TOOLS = [
    {
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "vendor":               {"type": "string", "description": "Exacte vendor-naam"},
                ...
                "product_status":       {"type": "string", "enum": ["active", "draft", "archived"]},
...
Je hebt toegang tot de Shopify product database via tools.

Gedragsregels:
- Gebruik altijd eerst zoek_producten om data op te halen voordat je conclusies trekt.
...
- Taal: Nederlands.
- Als een vraag gaat over producttitels, meta titles of descriptions: gebruik de juiste kolom.
- product_title = de interne productnaam in onze database
- current_meta_title = de SEO title die live staat in Shopify
"""
...
def _uitvoer_zoek(params: dict) -> list[dict]:
    ...
    if params.get("product_status"):
        ...
        q = q.eq("product_status", "active")  # standaard alleen actieve producten
        ...
        for tc in tool_calls:
            if tc.name == "zoek_producten":
                try:
                    ...
                    result_str = f"{len(data)} producten gevonden.\n" + str(data[:100])
...
def render() -> None:
    ...
    with st.expander("Voorbeeldvragen"):
        st.markdown(
            ...
            "- Toon alle Pottery Pots producten zonder meta description\n"
            "- Welke meta titles zijn langer dan 58 tekens?\n"
            "- Hoeveel producten hebben een templated description?\n"
            "- Verwijder het dubbele 'Serax' uit de gevonden titels\n"
            "- Toon alle producten van vendor 'Serax' die actief zijn"
        )
