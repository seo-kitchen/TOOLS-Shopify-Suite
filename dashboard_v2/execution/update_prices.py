...
Bevat ook een pure-Python entrypoint `run_price_update(...)` voor gebruik
...
class PriceUpdateError(Exception):
    """Raised when the price update pipeline cannot proceed."""
...
class PriceUpdateResult:
    matched_count: int = 0
    updated_count: int = 0
    not_found_rows: list[dict] = field(default_factory=list)
    hextom_xlsx_path: Optional[str] = None
    crx_xlsx_path: Optional[str] = None
...
def update_database(sb, updates: list[dict], dry_run: bool = False) -> int:
    """Update prijzen in Supabase. Vult rrp, inkoop, verkoopprijs, en gb-velden.

    Returns het aantal succesvol geüpdatete rijen (0 bij dry-run).
    """
    if dry_run:
...
def write_crx_overview(matched: list[dict], not_in_db: list[dict], output_path: Path) -> dict:
    """Genereer CRX prijsoverzicht: complete FKI-prijzen met giftbox-logica.

    Returns een dict met tellingen (increases/decreases/unchanged/newly_filled).
    """
    wb = openpyxl.Workbook()
...
def run_price_update(
    ...
    """Run de volledige prijs-update pipeline en geef een PriceUpdateResult terug.

    Preserves the giftbox rule: if giftbox_qty > 1, use rrp_gb_eur (NEVER stuksprijs).
    """
    ...
    pad = Path(file_path)
    if not pad.exists():
        raise PriceUpdateError(f"Bestand niet gevonden: {pad}")
    ...
    _progress(1, TOTAL_STEPS, "Prijsbestand laden")
    log(f"Prijsbestand laden: {pad}")
    try:
        df = load_price_file(str(pad))
    except Exception as e:
        raise PriceUpdateError(f"Kan Excel niet lezen: {e}") from e

    # 2. Prijzen extraheren (giftbox-logica)
    _progress(2, TOTAL_STEPS, "Prijzen extraheren (giftbox-logica)")
    ...
    for _, row in df.iterrows():
        ...
        if extracted["is_giftbox_set"]:
            gb_count += 1
    log(f"  {len(price_data)} unieke SKUs, {gb_count} giftbox-sets")

    # 3. Database ophalen
    _progress(3, TOTAL_STEPS, "Database producten ophalen")
    try:
        sb = get_supabase()
        db_products = fetch_db_products(sb)
    except Exception as e:
        raise PriceUpdateError(f"Supabase ophalen mislukt: {e}") from e

    # 4. Matchen
    _progress(4, TOTAL_STEPS, "SKU matching")
    matched: list[dict] = []
    not_in_db: list[dict] = []
    ...
    log(f"  Matched: {len(matched)} | Niet in DB: {len(not_in_db)}")

    # 5. Database updaten
    _progress(5, TOTAL_STEPS, "Database update" + (" (dry-run)" if dry_run else ""))
    try:
        updated = update_database(sb, matched, dry_run=dry_run)
    except Exception as e:
        raise PriceUpdateError(f"DB update fout: {e}") from e

    # 6. Hextom export
    _progress(6, TOTAL_STEPS, "Hextom bulk update Excel")
    hextom_path = out_dir / f"Hextom_Price_Update_{timestamp}.xlsx"
    try:
        write_hextom_price_update(matched, hextom_path)
    except Exception as e:
        raise PriceUpdateError(f"Hextom export mislukt: {e}") from e

    # 7. CRX overzicht
    _progress(7, TOTAL_STEPS, "CRX prijsverhogingsoverzicht")
    crx_path = out_dir / f"CRX_Prijsverhoging_Serax_{timestamp}.xlsx"
    try:
        crx_summary = write_crx_overview(matched, not_in_db, crx_path)
    except Exception as e:
        raise PriceUpdateError(f"CRX overzicht mislukt: {e}") from e

    gb_matched = sum(1 for m in matched if m.get("is_giftbox_set"))
    summary = {
        "total_rows": len(price_data),
        "matched": len(matched),
        "not_in_db": len(not_in_db),
        "giftbox_sets": gb_matched,
        "dry_run": dry_run,
        **crx_summary,
    }
    ...
    return PriceUpdateResult(
        ...
        not_found_rows=not_in_db,
        hextom_xlsx_path=str(hextom_path),
        crx_xlsx_path=str(crx_path),
        summary=summary,
    )
...
def main():
    ...
    print(f"Hextom : {result.hextom_xlsx_path}")
    print(f"CRX    : {result.crx_xlsx_path}")
