...
class ExportError(Exception):
    """Raised when export_standaard cannot proceed."""
    pass
...
def export_standaard(
    ...
    if progress is not None:
        progress(1, 4, "Producten ophalen uit Supabase")
    ...
    if not producten:
        ...
        return ExportResult(xlsx_path=None, nieuw_count=0, archief_count=0)
    ...
    # Tab 1: Nieuw
    if progress is not None:
        progress(2, 4, "Tab Shopify_Nieuw schrijven")
    ...
    log(f"  Bestand:  {pad}")
    log(f"  Nieuw:    {n_nieuw} producten (tab Shopify_Nieuw)")
    log(f"  Archief:  {n_archief} producten (tab Shopify_Archief)")
    log(f"  Analyse:  {len(alle)} producten totaal (tab Analyse)")

    return ExportResult(
        xlsx_path=str(pad),
        nieuw_count=n_nieuw,
        archief_count=n_archief,
    )
...
if __name__ == "__main__":
    ...
    try:
        export_standaard(args.fase, args.output, progress=None, logger=print)
